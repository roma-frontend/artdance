...
|---|---|---|
| **Видео 21,6 МБ на первом экране** | `hero-loop` | Кодировать по `videoProcessing.heroLoop`. С 05.09.2026 это не петля, а клип раскрытия занавеса: ≤ 2,4 МБ на файл, 10 с, версии 1280 и 1920, av1/vp9/h264, без звука, плотные ключевые кадры |
| **`autoplay` без `poster` и `preload`** | `.hero-video-main` | Постер обязателен. Автозапуска нет вовсе: клип стоит на первом кадре до жеста прокрутки |
| **Трейл до шести копий `<video>`** | `.hero-ghosts` | Удалён 05.09.2026 вместе с играющей петлёй: копии стоящего кадра читаются как дефект отрисовки. См. `docs/00-decision-record.md` §8b |
| **Нет кнопки паузы у видео** | hero | Не требуется: движения без действия пользователя не происходит |
