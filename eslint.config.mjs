...
const config = [
...

  {
    ignores: [
      ...
      'src/generated/**',
      /*
       * Служебные каталоги инструментов: вложенный чекаут kilo содержит копию
  ...
  /**
   * `Media` — единственный компонент, которому разрешён `next/image`, а
   * `HeroVideo` / `EditorialVideo` / `VideoPlayer` — единственные, кому разрешён
   * `<video>`. Исключение объявлено здесь, а не комментарием-отключением в
   * файле: так его видно в одном месте и нельзя размножить копипастой.
   *
   * Фоновых петель две, потому что секций две, и требования правила в обеих
   * выполнены каждое: постер показывается всегда, автозапуск подавляется при
   * reduced-motion и Save-Data, а роль управления движением (WCAG 2.2.2)
   * выполняет системная настройка — решение заказчика, см. шапки компонентов.
   */
  {
    files: [
  ...
  /**
   * FX-компоненты с canvas-графикой: цвета частиц собираются динамически из
   * золотых токенов (gold.200/500) + альфы кадра. Литеральный rgba здесь —
   * не токен-ошибка, а техника canvas API.
   */
  {
    files: ['src/components/fx/hero-mythic-dust.tsx'],
    rules: {
      'no-restricted-syntax': 'off',
    },
  },
