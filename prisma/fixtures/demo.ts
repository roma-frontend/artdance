...
/**
 * Слоты, зачёркнутые в макете как занятые. В прототипе это `text-decoration:
 * line-through` у трёх кнопок; здесь — данные, потому что «занято» приходит из
 * `Booking` и `SlotHold`, а не из вёрстки.
 */
export const demoTakenSlots = ['10:00', '11:30', '21:00'] as const;
