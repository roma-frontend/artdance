/**
 * PARTNERS — партнёры платформы (DS-06).
 *
 * Контентная страница с тремя формами партнёрства. Списка конкретных партнёров
 * пока нет (ни одного подтверждённого), и честнее показать, как партнёрство
 * устроено, чем вымышленный логотипный ряд: строки появят вместе с первыми
 * договорами, правка — контент-слой.
 */

import type { Metadata } from "next";
import { HandshakeIcon } from "lucide-react";
import { getTranslations, setRequestLocale } from "next-intl/server";

import { ContentSection } from "@/components/content/content-section";
import { PageHero, breadcrumbsFromTrail } from "@/components/layout/page-hero";
import { SiteFooter } from "@/components/layout/site-footer";
import { Button } from "@/components/ui/button";
import { routes, site } from "@/config";
import type { Locale } from "@/i18n/config";
import { Link } from "@/i18n/routing";
import type { Crumb } from "@/lib/seo/jsonld";
import { buildMetadata } from "@/lib/seo/metadata";
import { getDancesportHero } from "@/server/content/catalog";

interface PageProps {
  params: Promise<{ locale: string }>;
}

export async function generateMetadata({
  params,
}: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({
    locale: locale as Locale,
    namespace: "seo.partners",
  });

  return buildMetadata({
    locale: locale as Locale,
    path: routes.partners(),
    title: t("title"),
    description: t("description"),
  });
}

export default async function PartnersPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale as Locale);

  const t = await getTranslations("partners");

  const trail: Crumb[] = [{ name: t("eyebrow"), path: routes.partners() }];

  const tiers = [
    { key: "tierStudio", icon: HandshakeIcon },
    { key: "tierFederation", icon: HandshakeIcon },
    { key: "tierBrand", icon: HandshakeIcon },
  ] as const;

  return (
    <main id={site.mainContentId}>
      <PageHero
        title={t("title")}
        subtitle={t("subtitle")}
        eyebrow={t("eyebrow")}
        image={getDancesportHero("partners")}
        locale={locale as Locale}
        breadcrumbs={breadcrumbsFromTrail(trail)}
      />

      <ContentSection title={t("tiersTitle")}>
        <ul className="grid gap-6 md:grid-cols-3">
          {tiers.map(({ key, icon: Icon }) => (
            <li
              key={key}
              className="flex flex-col rounded-xl border border-border-default bg-surface-card p-8"
            >
              <Icon className="size-6 text-content-accent" aria-hidden />
              <h3 className="text-card-title mt-4">{t(`${key}.title`)}</h3>
              <p className="text-body-sm mt-3 text-content-secondary">
                {t(`${key}.body`)}
              </p>
            </li>
          ))}
        </ul>

        <div className="mt-10 text-center">
          <Button asChild size="lg" variant="accent">
            <Link href={routes.contact()}>{t("becomeCta")}</Link>
          </Button>
        </div>
      </ContentSection>

      <SiteFooter />
    </main>
  );
}
