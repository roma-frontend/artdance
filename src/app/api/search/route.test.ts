...
describe('GET /api/search — отказы', () => {
  ...
  it('превышение частоты отвечает 429 и говорит, когда повторить', async () => {
     ...
     * что лимитер не работает. Запас в один запрос сверх правила: первый
     * отклонённый и есть ответ, который проверяется.
     */
    for (let index = 0; index <= rateLimits.search.requests; index += 1) {
