  ...
  return (
    <span ref={groupRef} data-counter>
      {/*
        ...
        `suppressHydrationWarning` — не заглушка предупреждения, а признание
        факта: данные ICU в Node и в браузере не совпадают. Для локали `hy` Node
        отдаёт «2500» и «4,9», а Chrome — «2,500» и «4.9», и на армянской версии
        это давало ошибку гидратации (React #418): React отбрасывал серверную
        разметку и перерисовывал страницу целиком, то есть SSR терялся ровно там,
        где он нужнее всего. Здесь расхождение безопасно: текст этого узла
        всё равно перезаписывается анимацией через `textContent`, а до неё на
        экране стоит значение, отформатированное сервером.
      */}
      <span ref={numberRef} suppressHydrationWarning className={cn('tabular-nums', className)}>
        {render(value)}
      </span>
      {suffix}
    </span>
