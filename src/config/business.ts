...
export const commerce = {
  ...
  lowStockThreshold: 5,
  /**
   * Категория подарочных карт.
   *
   * Признак «продаётся от суммы» выводится из категории, а не хранится флагом у
   * товара: два источника правды об одном факте однажды разойдутся, и товар
   * окажется в разделе подарочных карт с фиксированной ценой либо наоборот.
   * Слаг — настройка, потому что категории заводит заказчик.
   */
  giftCardCategorySlug: 'gift-cards',
...
export const limits = {
  pagination: {
    ...
    /**
     * Сколько отзывов показывает страница сущности.
     *
     * Отзывы — не листинг: они подтверждают решение, а не заменяют его. Десять
     * закрывают вопрос «что пишут», а остальные уходят за «показать ещё», когда
     * оно появится. Без предела страница инструктора с тысячей отзывов
     * превращается в мегабайт разметки, который никто не дочитает.
     */
    reviewsPerPage: 10,
  },
  /**
   * Предел строк, которые запрос каталога забирает из базы за раз.
   *
   * Нужен, пока часть отбора считается в приложении: текстовый запрос сравнивает
   * ключи поиска с транслитерацией, и `ILIKE` его не заменяет (см.
   * `src/server/queries/`). Выборка без предела — это отказ в обслуживании,
   * который приходит не с атакой, а с ростом каталога: сначала «немного
   * медленнее», потом память.
   *
   * Число заведомо больше любого разумного каталога Еревана и заведомо меньше
   * того, что положит процесс. Уедет вместе с переходом отбора в индекс (2.8).
   */
  query: {
    maxRows: 500,
  },
  search: {
    ...
    popularCount: 5,
    /**
     * Сколько направлений предлагать рядом с результатами при введённом запросе.
     *
     * Меньше, чем в пустом состоянии: направление — это подборка, а не ответ на
     * запрос, и вытеснять ею конкретные занятия нельзя. Запрос вроде «танец»
     * задевает сразу несколько направлений, и без предела список подсказок
     * оказывался бы длиннее списка находок.
     */
    maxStyleSuggestions: 3,
    /**
     * Сколько результатов возвращает `/api/search` за один запрос.
     *
     * Больше, чем `maxSuggestions`: оверлей показывает первые строки, а страница
     * `/discover?q=` — всё найденное с разбивкой по разделам, и ей нужен запас,
     * иначе счётчики у переключателя разделов врут.
     */
    maxResults: 60,
    /**
     * Допуск на опечатку: «бочата» обязана находить «бачату».
     *
     * Порог применяется только к словам не короче `minWordLength` — на коротких
     * одна правка связывает слова, у которых нет ничего общего («low» → «new»),
     * и выдача перестаёт зависеть от запроса.
     */
    typo: { minWordLength: 5, maxEdits: 1 },
  },
  /**
   * Сколько карточек показывает хаб направления до ссылки «все».
   *
   * Хаб — не листинг: его задача ответить «этому здесь учат, вот кто и где», а
   * не выдать всё. Числа подобраны под сетку, чтобы последний ряд не был
   * оборванным: занятия идут по три в ряд, преподаватели и залы — по три.
   */
  styleHub: {
    classes: 6,
    instructors: 3,
  ...
  /**
   * Сколько ближайших свободных времён предлагать вместо «мест нет» (C-02).
   *
   * Три — не круглое число, а предел выбора без раздумий: список из десяти
   * альтернатив возвращает человека в ту же точку, из которой он вышел, — надо
   * снова выбирать. Три закрывают вопрос одним взглядом.
   */
  alternativeSlots: 3,
  upload: {
...
export const rateLimits = {
  ...
  /**
   * Доступность слотов. Лимит щедрее, чем у удержания: смотреть свободное время
   * — обычное поведение (перебрать месяц, вернуться назад), а занимать слот —
   * нет. Но и не бесконечный: расчёт разворачивает расписание на горизонт.
   */
  availability: { requests: 60, windowSeconds: 300 },
