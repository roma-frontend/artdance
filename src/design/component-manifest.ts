...
export const componentManifest: readonly ComponentSpec[] = [
  ...
  /* ───────────────────────── Главная ───────────────────────── */
  {
    ...
    role: 'Первый экран: живая фоновая петля, заголовок, две CTA, четыре показателя',
    wave: 'foundation',
    ...
    notes:
      'Версия 21.09.2026: занавес, раскрываемый прокруткой, удалён (решение заказчика — ' +
      'спортивный образ; приём с приколотым экраном блокировал прокрутку до конца клипа — ' +
      'жалоба «один скролл — и дожидайся видео»). Первый экран — обычная секция высотой в ' +
      'одно окно с живой фоновой петлёй (см. HeroVideo); HeroParallax с его полосой разгона ' +
      'удалён, слоя data-parallax нет, вуаль стоит в постоянной плотности (motion.heroScrim). ' +
      ...
      'используем 100dvh, иначе экран дёргается при появлении адресной строки на iOS. ' +
      'Утилита relative у секции ОБЯЗАТЕЛЬНА: абсолютные дети (кадр, вуаль) ищут точку ' +
      'отсчёта в ней — прежде её нарочно не ставили ради position: sticky, которого больше нет.',
  },
  {
    name: 'HeroVideo',
    path: 'components/home/hero-video.tsx',
    role: 'Кадр первого экрана: живая фоновая петля и постер',
    wave: 'foundation',
    ...
    notes:
      'Версия 21.09.2026: занавес, отматываемый прокруткой, удалён; петля снова играет сама ' +
      '(решение заказчика — спортивный образ). Скраб несовместим с быстрым движением даже с ' +
      'плотными ключевыми кадрами: движение читается перелистыванием; медленный классический ' +
      'кадр это прощал, спортивному это противопоказано. Компонент повторяет EditorialVideo ' +
      'с двумя отличиями: постер в пресете heroFullBleed и preloadAheadViewports: 0 — первый ' +
      'экран виден при открытии страницы, упреждать нечего. Управление воспроизведением — ' +
      'useBackgroundVideo: старт по canplay, пауза вне видимости и в фоновой вкладке. ' +
      ...
      'ПЕРЕВЁРНУТОЙ КОПИИ КЛИПА (heroReverseLoop) БОЛЬШЕ НЕТ: она обслуживала обратный проход ' +
      'раскрытия, которого больше нет. ' +
      ...
      'КНОПКИ ПАУЗЫ НЕТ и не требуется: роль управления движением выполняет системная ' +
      'настройка — то же решение, что у editorial-секции. ' +
  ...
  /*
   * HeroParallax удалён 21.09.2026 вместе с занавесом: у расслоения первого экрана
   * не осталось ни движения, ни слоёв — текст не двигается и не гаснет, вуаль стоит
   * в постоянной плотности (motion.heroScrim), кадр живёт в HeroVideo.
   */
  {
    ...
    notes:
      'Единственный расслоённый параллакс в продукте: на первом экране он больше не ' +
      'применяется (занавес удалён 21.09.2026). Ход считается от положения самой ' +
