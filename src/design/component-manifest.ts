...
export const componentManifest: readonly ComponentSpec[] = [
  ...
  },
  {
    ...
    notes:
      ...
      'Построена на Sheet (Radix Dialog): ловушка фокуса, Esc, клик по затемнению, блокировка ' +
      'прокрутки, aria-modal и скрытие остальной страницы от скринридера. z-index — из ' +
      'zIndex.drawer, а не 9999, как в макете. ' +
      'Полоска-«ручка» сверху — основная цель для закрытия: жест «смахнуть вниз» Radix не ' +
      'поддерживает, а крестик в углу на широком телефоне так же недостижим, как список у ' +
      'верхнего края. ' +
      'ДЕФЕКТ, найденный заказчиком: вендорный Sheet рассчитывает на утилиты animate-in и ' +
      'slide-in-from-bottom из пакета tw-animate-css, которого в проекте нет. Классы в ' +
      'разметке были, CSS они не генерировали, и шторка появлялась мгновенным щелчком — ' +
      'Tailwind на несуществующую утилиту не жалуется. Анимация объявлена в globals.css по ' +
      'data-slot и data-state (вендорные файлы не патчим), направление берётся из data-side. ' +
      'Открытие 500ms, закрытие 300ms: уходящая за полсекунды панель читается как подвисшая. ' +
      'Факт наличия анимации проверяется e2e, а не наличие класса.',
  },
  {
    name: 'SearchOverlay',
