...
export const componentManifest: readonly ComponentSpec[] = [
  ...
  },
  {
    ...
    notes:
      ...
      'ПРОИЗВОДИТЕЛЬНОСТЬ, три решения против рывков: (1) копии трейла — снимки кадра на ' +
      'canvas, а не вторые <video>: семь параллельных декодов одного файла не выдерживает ' +
      'ни один аппаратный декодер, остальные уходят на процессор и роняют кадры вместе с ' +
      'прокруткой; фильтр обесцвечивания запекается в растр, а не висит CSS-свойством; ' +
      '(2) источник выбирается через navigator.mediaCapabilities по признакам smooth и ' +
      'powerEfficient, а не по порядку <source>: AV1 легче на 500 KB, но без аппаратного ' +
      'декодера он дороже H.264 в разы; (3) петля играет только пока видна и пока вкладка ' +
      'активна (useBackgroundVideo), и стартует после canplay, а не с первых байтов. ' +
