 ...
 */
/**
 * Шлейф отстающих копий кадра за фоновой петлёй (`hero-ghosts` в прототипе).
 *
 * Значения перенесены из макета один в один, включая связку двух чисел, которая
 * там не описана, но критична: обёртка видео шире контейнера на
 * `wrapWidthPercent` (135%), а сдвиг задаётся в процентах ОТ ЕЁ ШИРИНЫ
 * (`slidePercent`, 25%). 1.35 − 1.35 × 0.25 = 1.0125 — правый край остаётся за
 * пределами контейнера с запасом 1,25%. Если поменять одно число, не пересчитав
 * другое, у правой кромки появляется вертикальная полоса.
 */
export const heroGhostTrail = {
  ...
  /** Ширина обёртки видео в процентах от контейнера. */
  wrapWidthPercent: 135,
  /** Сдвиг обёртки в процентах от её собственной ширины. */
  slidePercent: 25,
  slideMs: 14_000,
  returnMs: 2_500,
  fadeAllMs: 3_000,
  pauseMs: 600,
  /** Появление копии, её угасание и задержка до удаления из DOM. */
  ghostFadeInMs: 1_800,
  ghostFadeOutMs: 2_200,
  ghostRemoveAfterMs: 2_300,
  /** Совместное угасание всех копий перед возвратом кадра. */
  fadeAllOutMs: 2_500,
  /** Пауза перед первым циклом — чтобы петля успела показать первый кадр. */
  startDelayMs: 800,
  /** Запас после окончания сдвига, прежде чем перестать рождать копии. */
  stopSpawnAfterSlideMs: 200,
  /** Хвост цикла перед повтором. */
  cycleTailMs: 500,
  /** Кривая сдвига из макета: почти линейная, с мягкими концами. */
  slideEasing: 'cubic-bezier(0.25, 0.1, 0.25, 1)',
} as const;
