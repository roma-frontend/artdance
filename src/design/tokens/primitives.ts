...
/**
 * Размытие подложки (`backdrop-blur-*`). `hairline` — едва заметное матовое
 * стекло кольца-курсора: фон под ним читается, но край кольца не теряется.
 */
export const blur = {
  hairline: '1px',
} as const;
...
export const scrim = {
   ...
   *
   * Отдельное от `heroDiagonal`, потому что задача у него другая: диагональ
   * рассчитана на снимок с известной композицией, а здесь кадр — живая петля, и
   * яркость в области заголовка меняется от кадра к кадру.
   *
   * Горизонтальная вуаль держит текстовую колонку слева и отпускает кадр правее
   * 70% ширины, где в кадре стоит танцор: гасить его целиком значило бы получить
   * тёмное пятно вместо спортивной сцены. Плотности подобраны под САМЫЙ СВЕТЛЫЙ
   * кадр петли — прочие светлее не станут.
   */
  heroWide:
    'linear-gradient(100deg, rgba(11,10,9,0.94) 0%, rgba(11,10,9,0.86) 26%, rgba(11,10,9,0.68) 45%, rgba(11,10,9,0.30) 62%, rgba(11,10,9,0) 74%)',
  /**
   ...
   * перестаёт делиться на «текст слева, танцор справа»: текст занимает всю
   * ширину. Вуаль густеет к низу, где стоят показатели и поисковая строка, и
   * легче сверху, где заголовок лежит на тёмной части кадра.
   */
  heroNarrow:
    'linear-gradient(180deg, rgba(11,10,9,0.62) 0%, rgba(11,10,9,0.74) 34%, rgba(11,10,9,0.90) 62%, rgba(11,10,9,0.95) 100%)',
  editorialRadial:
