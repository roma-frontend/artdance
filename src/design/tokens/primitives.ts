 ...
 *  300 `#E58C99` / 350 `#F26A79` — светлые шаги: ТОЛЬКО как цвет текста в тёмной
 *      теме и поверх кинематографичных плоскостей. Заливкой не бывают.
 *  400 `#6E1422` — «signal red» (живая доступность, urgency, alert)
 *  500 `#6e1422` — наведение на тёмном фоне
 ...
 * Шаги 300 и 350 существуют не «для полноты рампы»: бургунди на почти чёрном
 * даёт около 2:1, поэтому у текстовых ролей тёмной темы обязан быть свой шаг —
 * пары проверяются `contrast.test.ts`, страница целиком — axe в e2e.
 */
export const crimson = {
   ...
   *
   * Заливка `signal` (шаг 400) как текст на почти чёрном не проходит AA. Шаг 300
   * по контрасту подошёл бы, но он розовый и в тёмной теме сливается с акцентным
   * текстом, а «осталось мало мест» обязано отличаться от цены. Этот шаг
   * сохраняет красный тон и проходит порог.
   */
  350: '#F26A79',
  400: '#6E1422',
  500: '#6e1422',
...
export const layout = {
  ...
  /**
   * Отступ сверху у полноэкранного поиска: поле ввода попадает в естественную
   * зону взгляда, а не в геометрический центр экрана. 15vh — из прототипа,
   * `dvh` вместо `vh` по той же причине, что у hero: адресная строка iOS меняет
   * `vh` на ходу, и поле дёргается при первой же прокрутке.
   */
  overlayTopOffset: '15dvh',
  /** Мобильное меню: 280px из прототипа. Уже, чем `sm`, — контент не растягивается. */
  drawerWidth: '17.5rem',
  /**
   * Правая колонка внутренних страниц: сводка брони, корзины и заказа.
   *
   * В прототипе это три разных числа на трёх экранах (360 / 380 / 400px), и они
   * не случайны: в сводке заказа к строкам итога добавляются миниатюры товаров,
   * а в сводке брони — таймер удержания слота. Поэтому токена три, а не один с
   * компромиссной серединой: колонка обязана вмещать своё содержимое без
   * переносов в строках «поле — значение».
   */
